...
# === CONFIG ===
SETS_TO_INCLUDE = ["mh3", "tdg", "ktk", "dtk", "frf"]
# SETS_TO_INCLUDE = [
#     "tstx",
#     "tinr",
#     "mh3",
...
SYNERGY_FILE = "random_real_synergies.json"
...
def s(value):
    """Scale a value based on UI_SCALE."""
    return int(value * UI_SCALE)


def sf(size_tuple):
    """Scale a font size (tuple of family, size, *options)."""
    family = size_tuple[0]
    try:
        # Use requested font
        ...
        # Fallback
        return ("Noto Sans Inscriptional Pahlavi", int(size_tuple[1] * UI_SCALE))
...
class SynergyApp:
    ...
    def display_cards(self):
        ...
        if key in self.synergy_map:
            ...
            if "synergy_edhrec" in existing:
                tags.append("edhrec :", existing["synergy_edhrec"])
            elif "synergy" in existing:
                tags.append("synthetic :", existing["synergy"])
    ...
    def label_synergy(self, value):
        ...
        # Enable next button always after labeling
        self.next_btn.config(state="normal")

        # Enable the other button, disable the clicked one
        if value == 1:
            self.yes_btn.config(state="disabled")
            self.no_btn.config(state="normal")
        else:
    ...
    def go_back(self):
        ...
        if previous:
            last = previous[0]
            c1 = next(
                (c for c in self.cards if c["name"] == last["card1"]["name"]), None
            )
            c2 = next(
                (c for c in self.cards if c["name"] == last["card2"]["name"]), None
            )
            if c1 and c2:
                self.current_pair = [c1, c2]
                self.display_cards()

                # Now update buttons based on the synergy_manual value
                synergy_value = last["synergy_manual"]
                if synergy_value == 1:
                    self.yes_btn.config(state="disabled")
                    self.no_btn.config(state="normal")
                elif synergy_value == 0:
                    self.no_btn.config(state="disabled")
                    self.yes_btn.config(state="normal")
...
def downlaod_all_images():
    ...
    for card in tqdm(cards, desc="Downloading images", unit="card"):
        name = card["name"].replace("/", "_").replace(" ", "_")
        image_path = os.path.join(IMAGE_CACHE_DIR, f"{name}.png")
